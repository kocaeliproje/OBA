...
void draw_window_buffered(window_t w) {
    for (int i = 0; i < w.h; i++) {
        for (int j = 0; j < w.w; j++) {
            ...
            back_buffer[index] = ' ';
            // İlk satır pencere başlığı (Gri), geri kalanı pencere gövdesi (Mavi)
            if (i == 0) back_buffer[index + 1] = 0x70; // Gri arka plan, beyaz yazı
            else back_buffer[index + 1] = 0x1E;        // Mavi arka plan, sarı yazı
        }
    }
}
...
void update_desktop_logic() {
    ...
    // 1. Eğer sol tuşa basıldıysa ve fare pencerenin "başlık barı" üzerindeyse sürüklemeyi başlat
    if (mouse_left_button) {
        if (!win1.is_dragging) {
            // Fare pencerenin üst kısmında mı? (Başlık barı kontrolü)
        ...
        win1.is_dragging = 0; // Tuş bırakıldıysa sürükleme biter
    }

    // 2. Eğer sürükleme aktifse pencereyi fareyle beraber hareket ettir
    if (win1.is_dragging) {
        win1.x = cursor_pos_x - (win1.w / 2); // Fareyi merkeze al (isteğe bağlı)
        win1.y = cursor_pos_y;
        
        // Ekran sınırlarını koru
        if (win1.x < 0) win1.x = 0;
        if (win1.x + win1.w > 79) win1.x = 80 - win1.w;
    }

    // update_desktop_logic fonksiyonu içindeki sınır kontrollerini güncelleyin
    if (win1.is_dragging) {
        win1.x = cursor_pos_x - (win1.w / 2);
         win1.y = cursor_pos_y;
         ...
         if (win1.x < 0) win1.x = 0;
            ...
            // Dikey Sınırlar (HAYATİ ÖNEMDE)
            if (win1.y < 0) win1.y = 0;
            if (win1.y + win1.h > 24) win1.y = 24 - win1.h; // Pencerenin altı görev çubuğuna çarpmasın
    }
}
/* 4. Ana Çizim Görevi */
void gorev_masaustu() {
    while(1) {
        update_desktop_logic(); // Fare ve pencere konumlarını hesapla
        
        clear_back_buffer();    // RAM'deki kopyayı temizle
        draw_desktop_buffered(); // Arka planı RAM'e çiz
        draw_window_buffered(win1); // Pencereyi RAM'e çiz
        
        // Fareyi en üste çiz (RAM'e)
        int mouse_index = (cursor_pos_y * 80 + cursor_pos_x) * 2;
        back_buffer[mouse_index] = 'X';
        back_buffer[mouse_index + 1] = 0x0E;

        // RAM'deki bitmiş resmi ekrana (0xB8000) fırlat!
        finalize_render(); 

        sleep_ms(16); // Yaklaşık 60 FPS - Akıcı bir deneyim
    }
}
